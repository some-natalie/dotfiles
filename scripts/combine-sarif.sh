...
function merge_sarif() {
  ...
  local rules1=$(echo "$j1" | jq '.runs[0].tool.driver.rules')
  local rules2=$(echo "$j2" | jq '.runs[0].tool.driver.rules')
  local results1=$(echo "$j1" | jq '.runs[0].results')
  local results2=$(echo "$j2" | jq '.runs[0].results')
  ...
  local combined_rules=$(jq -s '.[0] + .[1]' <(echo "$rules1") <(echo "$rules2"))
  local unique_rules=$(echo "$combined_rules" | jq 'unique_by(.id)')

  echo "Number of rules combined is: $(echo "$unique_rules" | jq 'length')"

  local combined_results=$(jq -s '.[0] + .[1]' <(echo "$results1") <(echo "$results2"))
